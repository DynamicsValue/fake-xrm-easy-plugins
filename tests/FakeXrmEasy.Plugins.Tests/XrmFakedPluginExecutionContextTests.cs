...
namespace FakeXrmEasy.Abstractions.Tests.Plugins
{
    public class XrmFakedPluginExecutionContextTests
    {
        ...
        public void Should_generate_plugin_context_from_a_serialised_profile()
        {
            ...
            var pluginContext = XrmFakedPluginExecutionContext.FromSerialisedAndCompressedProfile(serialised);

            Assert.NotEqual(0, pluginContext.Stage);
            Assert.Equal("orga6b1c653", pluginContext.OrganizationName);
            Assert.Equal("Create", pluginContext.MessageName);
            Assert.Equal("account", pluginContext.PrimaryEntityName);
            Assert.Equal((int) ProcessingStepStage.Preoperation, pluginContext.Stage);

            Assert.True(pluginContext.InputParameters.ContainsKey("Target"));
